...
import { getValidateMessages } from '../src/infrastructure/validators/helpers.validators';

describe('Validators', () => {
  describe('standardString', () => {
    it('should validate string length', () => {
      const config = [
        [['name'], [[(value: any) => value.length >= 3, 'Invalid length']]],
      ];
  ...
  describe('standardEmail', () => {
    it('should validate email format', () => {
      const config = [
        [
          ['email'],
          [[(value: any) => /\S+@\S+\.\S+/.test(value), 'Invalid email']],
        ],
      ];
  ...
  describe('standardNaturalNumber', () => {
    it('should validate natural number', () => {
      const config = [
        [['age'], [[(value: any) => /^\d+$/.test(value), 'Invalid number']]],
      ];
  ...
  describe('standardKey', () => {
    it('should validate key format', () => {
      const config = [
        [
          ['key'],
          [[(value: any) => /^[a-zA-Z0-9-_]+$/.test(value), 'Invalid key']],
        ],
      ];
    ...
    it('should validate URL format', () => {
      const config = [
        [
          ...
          [
            [
              (value: any) => /^(https?:\/\/[^\s$.?#].[^\s]*)$/.test(value),
              'Invalid URL',
            ],
          ],
        ],
  ...
  describe('array', () => {
    it('should validate array elements', () => {
      ...
      const messages = getValidateMessages(config, {
        emails: ['valid@example.com', 'invalidemail'],
      });
      expect(messages).toContain('Invalid email');

      const validMessages = getValidateMessages(config, {
        emails: ['valid@example.com', 'another@example.com'],
      });
      expect(validMessages).toEqual([]);
    });
  });
...
describe('envValidators', () => {
  ...
  it('should validate projectKey as a valid string', () => {
    ...
    expect(validMessage).toEqual([]);
    expect(invalidMessage).toContainEqual(
      expect.objectContaining({ code: 'InvalidProjectKey' })
    );
  });
  ...
  it('should validate storeKey as a valid string', () => {
    ...
    expect(validMessage).toEqual([]);
    expect(invalidMessage).toContainEqual(
      expect.objectContaining({ code: 'InvalidStoreKey' })
    );
  });

  it('should validate relewise datasetId as a valid string', () => {
    const config = envValidators.find(
      (v) => v[0][0] === 'relewise' && v[0][1] === 'datasetId'
    );
    const validMessage = getValidateMessages([config], {
      relewise: { datasetId: 'valid-dataset-id' },
    });
    const invalidMessage = getValidateMessages([config], {
      relewise: { datasetId: 'invalid dataset id!' },
    });

    expect(validMessage).toEqual([]);
    expect(invalidMessage).toContainEqual(
      expect.objectContaining({ code: 'InvalidDatasetId' })
    );
  });

  it('should validate relewise apiKey as a valid string', () => {
    const config = envValidators.find(
      (v) => v[0][0] === 'relewise' && v[0][1] === 'apiKey'
    );
    const validMessage = getValidateMessages([config], {
      relewise: { apiKey: 'valid-api-key' },
    });
    const invalidMessage = getValidateMessages([config], {
      relewise: { apiKey: 'invalid api key!' },
    });
  ...
  it('should validate relewise serverUrl as a valid URL', () => {
    const config = envValidators.find(
      (v) => v[0][0] === 'relewise' && v[0][1] === 'serverUrl'
    );
    const validMessage = getValidateMessages([config], {
      relewise: { serverUrl: 'http://example.com' },
    });
    const invalidMessage = getValidateMessages([config], {
      relewise: { serverUrl: 'invalid-url' },
    });

    expect(validMessage).toEqual([]);
    expect(invalidMessage).toContainEqual(
      expect.objectContaining({ code: 'InvalidServerUrl' })
    );
  });
});
