...
import {
  groupBy,
  localizedToLanguageLookUp,
  localizedToMultilingual,
  mapListPrice,
  mapSalesPrice,
  searchKeywordsToMultilingual,
} from './helpers';
...
export function mapProduct(
  ...
  const builder = new ProductUpdateBuilder({
    ...
    .data({
      ...
      InStock: DataValueFactory.boolean(
        variants.some((variant) => variant.availability?.isOnStock)
      ),
    })
      ...
      for (const categoryLink of product.categories) {
        ...
        if (category) {
          categoryBuilder.path((p) => {
            category.ancestors.forEach((parent) => {
              const parentCategory = categoriesMap.get(parent.id);
              if (parentCategory) {
                p.category({
                  id: parentCategory.id,
                  displayName: localizedToLanguageLookUp(parentCategory.name),
                });
              }
            });
            p.category({
              id: category.id,
              displayName: localizedToLanguageLookUp(category.name),
            });
          });
        }
      }
...
export function mapVariants(
  ...
  return variants.map((variant) => {
    const builder = new ProductVariantBuilder({
      id: variant.sku ?? variant.id.toString(),
    })
      .data({
        Id: DataValueFactory.number(variant.id),
        IsMasterVariant: DataValueFactory.boolean(
          variant.id === product.masterVariant.id
        ),
        ImageUrls: variant.images
          ? DataValueFactory.stringCollection(variant.images.map((x) => x.url))
          : null,
        InStock: DataValueFactory.boolean(
          variant.availability?.isOnStock ?? false
        ),
        AvailableQuantity: DataValueFactory.number(
...
export function mapPriceOnProduct(
  builder: ProductUpdateBuilder,
  variants: CTProductVariant[]
) {
  const lowestListPrice = Object.entries(
    groupBy(
      variants.flatMap((v) => v.prices?.map((p) => mapListPrice(p)) ?? []),
      (t) => t.currency
    )
  ).map((currencyGroup) => ({
    currency: currencyGroup[0],
    amount: currencyGroup[1].sort((a, b) => a.amount - b.amount)[0].amount,
  }));

  const lowestSalesPrice = Object.entries(
    groupBy(
